...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return {
        ...
        openGraph: {
            ...
            url: canonicalUrl,
            type: "website",
            siteName: locale === "en" ? "Rukn Coupons" : "ركن الكوبونات",
        },
        twitter: {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-8 xl:col-span-9">
                        ...
                        <h1 className="text-3xl font-black text-gray-800 mb-4">
                            {pageDef.content[locale].h1(countryName)}
                        </h1>

                        {/* Intro paragraph */}
                        <p className="text-gray-600 leading-relaxed mb-6 text-sm">
                            {pageDef.content[locale].intro(countryName)}
                        </p>

                        {/* Results count */}
                        <p className="text-sm text-gray-500 mb-4 font-medium">
                            {coupons.length} {isEn ? "coupons available" : "كوبون متاح"}
                        </p>
                        ...
                        {coupons.length > 0 ? (
                            ...
                            <div className="bg-white p-20 rounded-3xl text-center border-2 border-dashed border-gray-200">
                                ...
                                <p className="text-gray-500 mb-6">{isEn ? "Check back soon for new deals." : "تحقق قريباً من العروض الجديدة."}</p>
                                <Link href={`/${locale}/${country}`} className="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-blue-700 transition-colors inline-block">
                                    {isEn ? "Browse All Stores" : "تصفح جميع المتاجر"}
                                </Link>
                            </div>
                        )}
                        ...
                        <section className="mt-12 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {topStores.map(store => (
                                    ...
                                    >
                                        {store.logoUrl && (
                                            <img src={store.logoUrl} alt="" className="w-6 h-6 rounded-full object-contain" loading="lazy" width={24} height={24} />
                                        )}
                            ...
                            <Link
                                href={`/${locale}/${country}`}
                                className="mt-4 inline-block text-blue-600 hover:text-blue-800 font-bold text-sm"
                            >
                        ...
                        <section className="mt-12 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <Link href={`/${locale}/${country}/today-deals`} className="bg-white rounded-xl p-4 text-center hover:shadow-md transition-shadow font-bold text-sm text-gray-700 hover:text-blue-600">
                                    🔥 {isEn ? "Today's Deals" : "عروض اليوم"}
                                </Link>
                                <Link href={`/${locale}/${country}/new-coupons`} className="bg-white rounded-xl p-4 text-center hover:shadow-md transition-shadow font-bold text-sm text-gray-700 hover:text-blue-600">
                                    ✨ {isEn ? "New Coupons" : "كوبونات جديدة"}
                                </Link>
                                <Link href={`/${locale}/${country}/no-code-needed`} className="bg-white rounded-xl p-4 text-center hover:shadow-md transition-shadow font-bold text-sm text-gray-700 hover:text-blue-600">
